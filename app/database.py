...
async def save_user(telegram_id, full_name, phone, birthday, pinfl):
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            "INSERT OR REPLACE INTO users (telegram_id, full_name, phone, birthday, pinfl) VALUES (?, ?, ?, ?, ?)",
...
async def get_user_by_telegram_id(telegram_id: int):
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT full_name, phone, birthday, pinfl FROM users WHERE telegram_id = ?", (telegram_id,)) as cursor:
            return await cursor.fetchone()

# 🔍 Все пользователи
async def get_all_users():
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT telegram_id, full_name, phone, birthday, pinfl FROM users") as cursor:
