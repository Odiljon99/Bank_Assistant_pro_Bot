...
from app.messages import langs from app.keyboards import get_main_menu, get_language_keyboard, get_agree_keyboard from app.config import ADMINS from app.database import ( get_user_by_telegram_id, save_user, update_user_field )
...
@router.message(F.text.in_([langs["ru"]["menu"], langs["uz"]["menu"]])) async def main_menu(message: Message, state: FSMContext): data = await state.get_data() lang = get_lang(data) await message.answer(langs[lang]["menu"])
...
@router.message(F.text.in_([langs["ru"]["main_menu_options"][-1], langs["uz"]["main_menu_options"][-1]])) async def show_my_data(message: Message, state: FSMContext): data = await state.get_data() lang = get_lang(data) user_data = await get_user_by_telegram_id(message.from_user.id) if not user_data: return await message.answer("⛔️ Данные не найдены") full_name, phone, birthday, pinfl = user_data await message.answer( f"📛 ФИО: {full_name}\n📞 Телефон: {phone}\n📅 Дата рождения: {birthday}\n🆔 ПИНФЛ: {pinfl}\n\n{langs[lang]['edit_data_prompt']}", reply_markup=ReplyKeyboardMarkup( keyboard=[[KeyboardButton(text=field)] for field in ["ФИО", "Телефон", "Дата рождения", "ПИНФЛ"]] + [[KeyboardButton(text=langs[lang]["cancel"])]], resize_keyboard=True ) ) await state.set_state(EditFieldState.choosing)

@router.message(EditFieldState.choosing) async def choose_field_to_edit(message: Message, state: FSMContext): field_map = { "ФИО": "full_name", "Телефон": "phone", "Дата рождения": "birthday", "ПИНФЛ": "pinfl", "ФИО": "full_name", "Telefon": "phone", "Tug‘ilgan sana": "birthday", "PINFL": "pinfl" } chosen = message.text if chosen not in field_map: return await message.answer("❌ Неверный выбор") await state.update_data(edit_field=field_map[chosen]) await message.answer("✏️ Введите новое значение:") await state.set_state(EditFieldState.editing)

@router.message(EditFieldState.editing) async def update_field_value(message: Message, state: FSMContext): data = await state.get_data() field = data.get("edit_field") if not field: return await message.answer("⚠️ Ошибка. Поле не выбрано.") await update_user_field(message.from_user.id, field, message.text) lang = get_lang(data) await message.answer(langs[lang]["data_updated"], reply_markup=get_main_menu(lang)) await state.clear()
