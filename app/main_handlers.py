...
@router.message(EditFieldState.choosing) async def choose_field_to_edit(message: Message, state: FSMContext): data = await state.get_data() lang = get_lang(data) if message.text == langs[lang]["cancel"]: await message.answer(langs[lang]["menu"], reply_markup=get_main_menu(lang)) return await state.clear()
...
await message.answer("✏️ Введите новое значение:", reply_markup=get_back_keyboard(lang))
await state.set_state(EditFieldState.editing)

@router.message(EditFieldState.editing) async def update_field_value(message: Message, state: FSMContext): data = await state.get_data() lang = get_lang(data) if message.text == langs[lang]["back"]: await show_my_data(message, state) return
